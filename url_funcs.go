...
func makeURLJoinFunc() function.Function {
	return function.New(&function.Spec{
		...
		Type: function.StaticReturnType(URLObjectType),
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			base, err := GetURLFromValue(args[0])
			if err != nil {
				return cty.NilVal, fmt.Errorf("urljoin: base: %w", err)
			}
			ref, err := GetURLFromValue(args[1])
			if err != nil {
				return cty.NilVal, fmt.Errorf("urljoin: ref: %w", err)
			}
			return BuildURLObject(base.ResolveReference(ref)), nil
		},
	})
}
...
func makeURLJoinPathFunc() function.Function {
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			return BuildURLObject(base.JoinPath(elems...)), nil
		},
	})
}
